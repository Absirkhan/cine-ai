...
const HomeScreen = ({ onGenerate }) => {
  const [prompt, setPrompt] = React.useState("A lonely astronaut discovers a glowing crystal on Mars. As she picks it up, the rocks around her start to hum and pulse with light. She whispers: \"It's… alive.\"");
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      setGenerating(false);
      onGenerate && onGenerate({ prompt, genre, tone, duration, aspect });
    }, 700);
  };
...
const PromptSelect = ({ label, value, options, onChange, icon }) => {
