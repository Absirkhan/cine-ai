...
import type { ChatMessage, EditTarget, PlanStep } from '../types';
...
const EditAgent: React.FC = () => {
  ...
  const [detected, setDetected] = useState<EditTarget>({ type: null, detail: undefined });
  const [pending, setPending] = useState<boolean>(false);
  const listRef = useRef<HTMLDivElement>(null);

  // Memoize the onMessage callback to prevent WebSocket reconnections
  ...
  const { connected, sendMessage: wsSendMessage } = useChatWebSocket({
    runId: jobId!,
    onMessage: handleMessage,
    enabled: !!jobId,
  });

  // Fallback to mock chat if WebSocket is not connected
  useEffect(() => {
    ...
    setMessages([
      {
        role: 'system',
        content: `Edit Agent is watching ${jobId} (demo mode - WebSocket not connected). I can adjust script, audio, video frames, or pacing — just describe the change.`,
        time: '14:28:02',
      },
      {
        role: 'user',
  ...
  const send = (text?: string) => {
    ...
    const userMsg: ChatMessage = { role: 'user', content, time: now };
    ...
    if (connected) {
      ...
      // Fallback to mock reply if not connected
      const target = detectTarget(content);
      setTimeout(() => {
        const reply = buildReply(content, target);
...
// --- helpers ---
function detectTarget(text: string): EditTarget {
...
interface DetectedTargetChipProps {
  target: EditTarget;
}

const DetectedTargetChip: React.FC<DetectedTargetChipProps> = ({ target }) => {
  const colorMap: Record<string, { bg: string; bd: string; fg: string }> = {
  ...
  const c = colorMap[target.type || 'Script'] || colorMap.Script;
  return (
